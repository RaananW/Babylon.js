...
let mergeBase;
try {
    mergeBase = execSync(`git merge-base HEAD ${baseRef}`, { encoding: "utf-8" }).trim();
} catch {
...
const diffOutput = execSync(`git diff --name-only --diff-filter=d ${mergeBase}`, { encoding: "utf-8" }).trim();
...
const result = spawnSync("npx", eslintArgs, { stdio: "inherit", shell: true });
