 ...
 */
export async function NodeRenderGraphCreateDefaultAsync(name: string, scene: Scene, nodeRenderGraphOptions?: INodeRenderGraphCreateOptions): Promise<NodeRenderGraph> {
...
export function NodeRenderGraphParseFromSnippetAsync(
    snippetId: string,
    scene: Scene,
    ...
    if (snippetId === "_BLANK") {
        return NodeRenderGraphCreateDefaultAsync("blank", scene, nodeRenderGraphOptions);
    }
    ...
    return new Promise((resolve, reject) => {
        ...
        request.open("GET", NodeRenderGraph.SnippetUrl + "/" + snippetId.replace(/#/g, "/"));
        request.send();
    });
}
NodeRenderGraph;
