...
export class BoundingBox implements ICullable {
    ...
    public isInFrustum(frustumPlanes: Array<DeepImmutable<Plane>>): boolean {
        return BoundingBoxIsInFrustum(this.vectorsWorld, frustumPlanes);
    }
    ...
    public isCompletelyInFrustum(frustumPlanes: Array<DeepImmutable<Plane>>): boolean {
        return BoundingBoxIsCompletelyInFrustum(this.vectorsWorld, frustumPlanes);
    }
 ...
 * @returns true if there is an intersection
 */
export function BoundingBoxIntersects(box0: DeepImmutable<BoundingBox>, box1: DeepImmutable<BoundingBox>): boolean {
 ...
 * @returns true if there is an inclusion
 */
export function BoundingBoxIsCompletelyInFrustum(boundingVectors: Array<DeepImmutable<Vector3>>, frustumPlanes: Array<DeepImmutable<Plane>>): boolean {
    for (let p = 0; p < 6; ++p) {
 ...
 * @returns true if there is an intersection
 */
export function BoundingBoxIsInFrustum(boundingVectors: Array<DeepImmutable<Vector3>>, frustumPlanes: Array<DeepImmutable<Plane>>): boolean {
    for (let p = 0; p < 6; ++p) {
