...
import { SpriteSceneComponent } from "./spriteSceneComponent.pure";
...
export class SpriteManager implements ISpriteManager {
     ...
     */
    public static CreateFromSnippetAsync = SpriteManagerParseFromSnippetAsync;
}
 ...
 */
export function SpriteManagerParse(parsedManager: any, scene: Scene, rootUrl: string): SpriteManager {
 ...
 * @returns a promise that will resolve to the new sprite manager
 */
export async function SpriteManagerParseFromFileAsync(name: Nullable<string>, url: string, scene: Scene, rootUrl: string = ""): Promise<SpriteManager> {
        ...
        request.addEventListener("readystatechange", () => {
            if (request.readyState == 4) {
                if (request.status == 200) {
                    ...
                    const output = SpriteManagerParse(serializationObject, scene || EngineStore.LastCreatedScene, rootUrl);
...
export function SpriteManagerParseFromSnippetAsync(snippetId: string, scene: Scene, rootUrl: string = ""): Promise<SpriteManager> {
    if (snippetId === "_BLANK") {
