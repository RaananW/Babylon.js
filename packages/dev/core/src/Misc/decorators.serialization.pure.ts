...
const CopySource = function <T>(creationFunction: () => T, source: T, instanciate: boolean, options: ICopySourceOptions = {}): T {
    ...
    if (Tags && TagsHasTags(source)) {
        TagsAddTagsTo(destination, TagsGetTags(source, true));
    }
 ...
 */
export function SerializationHelperAppendSerializedAnimations(source: IAnimatable, destination: any): void {
 ...
 */
export function SerializationHelperSerialize<T>(entity: T, serializationObject?: any): any {
    ...
    // Tags
    if (Tags) {
        serializationObject.tags = TagsGetTags(entity);
    }
 ...
 */
export function SerializationHelperParseProperties(source: any, destination: any, scene: Nullable<Scene>, rootUrl: Nullable<string>) {
    if (!rootUrl) {
        rootUrl = "";
    }
 ...
 */
export function SerializationHelperParse<T>(creationFunction: () => T, source: any, scene: Nullable<Scene>, rootUrl: Nullable<string> = null): T {
    const destination = creationFunction();

    // Tags
    if (Tags) {
        TagsAddTagsTo(destination, source.tags);
    }

    SerializationHelperParseProperties(source, destination, scene, rootUrl);
 ...
 */
export function SerializationHelperClone<T>(creationFunction: () => T, source: T, options: ICopySourceOptions = {}): T {
