...
const SerializeMeshInternal = (mesh: Mesh, serializationScene: any): any => {
...
const FinalizeSingleNode = (node: Node, serializationObject: any) => {
    if ((node as Mesh)._isMesh) {
        ...
        if (mesh.delayLoadState === Constants.DELAYLOADSTATE_LOADED || mesh.delayLoadState === Constants.DELAYLOADSTATE_NONE) {
            ...
            serializationObject.meshes.push(SerializeMeshInternal(mesh, serializationObject));
        }
...
function _Serialize(scene: Scene, checkSyncReadSupported = true): any {
    const serializationObject: any = {};
...
function _CollectPromises(obj: any, promises: Array<Promise<any>>): void {
 ...
 */
export function SceneSerializerClearCache(): void {
    SerializedGeometries = [];
}
 ...
 */
export function SceneSerializerSerialize(scene: Scene): any {
 ...
 */
export async function SceneSerializerSerializeAsync(scene: Scene): Promise<any> {
    const serializationObject = _Serialize(scene, false);

    const promises: Array<Promise<any>> = [];

    _CollectPromises(serializationObject, promises);
 ...
 * @returns a JSON compatible object
 */
export function SceneSerializerSerializeMesh(toSerialize: any /* Mesh || Mesh[] */, withParents: boolean = false, withChildren: boolean = false): any {
    const serializationObject: any = {};
    serializationObject.meshes = [];
    serializationObject.transformNodes = [];
    serializationObject.cameras = [];
