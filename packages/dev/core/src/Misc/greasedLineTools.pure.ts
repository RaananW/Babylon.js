 ...
 */
export function GreasedLineToolsConvertPoints(points: GreasedLinePoints, options?: GreasedLinePointsOptions): number[][] {
 ...
 * @returns original points or null if any edge length is zero
 */
export function GreasedLineToolsOmitZeroLengthPredicate(p1: Vector3, p2: Vector3, p3: Vector3) {
    const fileredPoints = [];
    // edge1
 ...
 */
export function GreasedLineToolsMeshesToLines(
    meshes: AbstractMesh[],
    predicate?: (
        p1: Vector3,
        p2: Vector3,
 ...
 */
export function GreasedLineToolsToVector3Array(points: number[] | number[][]) {
 ...
 */
export function GreasedLineToolsToNumberArray(points: Vector3[]) {
 ...
 */
export function GreasedLineToolsGetPointsCountInfo(points: number[][]): { total: number; counts: number[] } {
 ...
 */
export function GreasedLineToolsGetLineLength(data: Vector3[] | number[]): number {
    if (data.length === 0) {
        return 0;
    }
    ...
    if (typeof data[0] === "number") {
        points = GreasedLineToolsToVector3Array(<number[]>data) as Vector3[];
    } else {
 ...
 */
export function GreasedLineToolsGetLineLengthArray(data: number[], buf?: ArrayBuffer): Float32Array {
 ...
 * @returns
 */
export function GreasedLineToolsSegmentizeSegmentByCount(point1: Vector3, point2: Vector3, segmentCount: number): Vector3[] {
 ...
 * @returns line point
 */
export function GreasedLineToolsSegmentizeLineBySegmentLength(
    what: Vector3[] | number[] | { point1: Vector3; point2: Vector3; length: number }[],
    segmentLength: number
): Vector3[] {
    const subLines =
        what[0] instanceof Vector3
            ? GreasedLineToolsGetLineSegments(what as Vector3[])
            : typeof what[0] === "number"
              ? GreasedLineToolsGetLineSegments(GreasedLineToolsToVector3Array(what as number[]) as Vector3[])
              : (what as { point1: Vector3; point2: Vector3; length: number }[]);
    const points: Vector3[] = [];
    for (const s of subLines) {
        if (s.length > segmentLength) {
            const segments = GreasedLineToolsSegmentizeSegmentByCount(s.point1, s.point2, Math.ceil(s.length / segmentLength));
 ...
 * @returns line point
 */
export function GreasedLineToolsSegmentizeLineBySegmentCount(what: Vector3[] | number[], segmentCount: number): Vector3[] {
    const points = <Vector3[]>(typeof what[0] === "number" ? GreasedLineToolsToVector3Array(<number[]>what) : what);
    const segmentLength = GreasedLineToolsGetLineLength(points) / segmentCount;
    return GreasedLineToolsSegmentizeLineBySegmentLength(points, segmentLength);
}
 ...
 */
export function GreasedLineToolsGetLineSegments(points: Vector3[]): { point1: Vector3; point2: Vector3; length: number }[] {
    const segments = [];
    for (let index = 0; index < points.length - 1; index++) {
        const point1 = points[index];
        const point2 = points[index + 1];
 ...
 * @returns
 */
export function GreasedLineToolsGetMinMaxSegmentLength(points: Vector3[]): { min: number; max: number } {
    const subLines = GreasedLineToolsGetLineSegments(points);
 ...
 */
export function GreasedLineToolsGetPositionOnLineByVisibility(
    lineSegments: { point1: Vector3; point2: Vector3; length: number }[],
    lineLength: number,
    visbility: number,
    localSpace = false
) {
 ...
 * @returns line points
 */
export function GreasedLineToolsGetCircleLinePoints(radiusX: number, segments: number, z = 0, radiusY = radiusX, segmentAngle = (Math.PI * 2) / segments) {
    const points: Vector3[] = [];
 ...
 * @returns
 */
export function GreasedLineToolsGetArrowCap(position: Vector3, direction: Vector3, length: number, widthUp: number, widthDown: number, widthStartUp = 0, widthStartDown = 0) {
 ...
 */
export function GreasedLineToolsGetPointsFromText(text: string, size: number, resolution: number, fontData: IFontData, z = 0, includeInner = true) {
    const allPoints = [];
 ...
 */
export function GreasedLineToolsColor3toRGBAUint8(colors: Color3[]): Uint8Array {
 ...
 */
export function GreasedLineToolsCreateColorsTexture(name: string, colors: Color3[], colorsSampling: number, scene: Scene) {
    ...
    const colorsArray = GreasedLineToolsColor3toRGBAUint8(colors);
 ...
 */
export function GreasedLineToolsPrepareEmptyColorsTexture(scene: Scene) {
 ...
 */
export function GreasedLineToolsDisposeEmptyColorsTexture() {
