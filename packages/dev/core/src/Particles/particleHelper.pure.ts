...
export class ParticleHelper {
     ...
     */
    public static CreateFromSnippetAsync = ParticleHelperParseFromSnippetAsync;
}
 ...
 */
export function ParticleHelperCreateDefault(emitter: Nullable<AbstractMesh | Vector3>, capacity = 500, scene?: Scene, useGPU = false): IParticleSystem {
...
export function ParticleHelperCreateAsync(type: string, scene: Nullable<Scene>, gpu: boolean = false, capacity?: number): Promise<ParticleSystemSet> {
    if (!scene) {
 ...
 */
export function ParticleHelperExportSet(systems: IParticleSystem[]): ParticleSystemSet {
 ...
 */
// eslint-disable-next-line @typescript-eslint/promise-function-async, no-restricted-syntax
export function ParticleHelperParseFromFileAsync(
 ...
 */
// eslint-disable-next-line @typescript-eslint/promise-function-async, no-restricted-syntax
export function ParticleHelperParseFromSnippetAsync(snippetId: string, scene: Scene, gpu: boolean = false, rootUrl: string = "", capacity?: number): Promise<IParticleSystem> {
    if (snippetId === "_BLANK") {
        const system = ParticleHelperCreateDefault(null);
        system.start();
    ...
    return new Promise((resolve, reject) => {
        ...
        request.open("GET", ParticleHelper.SnippetUrl + "/" + snippetId.replace(/#/g, "/"));
        request.send();
    });
}
...
/**
 * Register side effects for particleHelper.
 * Safe to call multiple times; only the first call has an effect.
 */
export function registerParticleHelper(): void {
    if (_registered) {
        return;
    }
    _registered = true;
