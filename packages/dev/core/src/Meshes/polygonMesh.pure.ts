 ...
 */
export function PolygonRectangle(xmin: number, ymin: number, xmax: number, ymax: number): Vector2[] {
 ...
 */
export function PolygonCircle(radius: number, cx: number = 0, cy: number = 0, numberOfSides: number = 32): Vector2[] {
 ...
 */
export function PolygonParse(input: string): Vector2[] {
    const floats = input
        .split(/[^-+eE.\d]+/)
        .map(parseFloat)
