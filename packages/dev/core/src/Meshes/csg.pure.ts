...
export class CSG {
     ...
     * @internal
     */
    static _FromPolygons(polygons: CSGPolygon[]): CSG {
        const csg = new CSG();
 ...
 * @returns the new CSG
 */
export function CSGFromVertexData(data: VertexData): CSG {
 ...
 */
export function CSGFromMesh(mesh: Mesh, absolute = false): CSG {
    let vertex: Vertex,
        normal: Vector3,
    ...
    const indices = mesh.getIndices(),
        ...
        normals2 = mesh.getVerticesData(VertexBuffer.NormalKind),
    ...
    for (let sm = 0, sml = subMeshes.length; sm < sml; sm++) {
        for (let i = subMeshes[sm].indexStart, il = subMeshes[sm].indexCount + subMeshes[sm].indexStart; i < il; i += 3) {
            ...
            for (let j = 0; j < 3; j++) {
                ...
                const sourceNormal = new Vector3(normals2[indices[indexIndices] * 3], normals2[indices[indexIndices] * 3 + 1], normals2[indices[indexIndices] * 3 + 2]);
                if (uvs) {
