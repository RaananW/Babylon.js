 ...
 */
export function NodeGeometryCreateDefault(name: string) {
...
export function NodeGeometryParseFromSnippetAsync(snippetId: string, nodeGeometry?: NodeGeometry, skipBuild: boolean = false): Promise<NodeGeometry> {
    if (snippetId === "_BLANK") {
        return Promise.resolve(NodeGeometryCreateDefault("blank"));
    }
