...
import { type IWebXRMotionControllerAxesValue, type IWebXRMotionControllerComponentChangesValues } from "../motionController/webXRControllerComponent"
...
export class WebXRControllerMovement extends WebXRAbstractFeature {
    ...
    constructor(_xrSessionManager: WebXRSessionManager, options: IWebXRControllerMovementOptions) {
        ...
        if (!options || options.xrInput === undefined) {
            ToolsError('WebXRControllerMovement feature requires "xrInput" option.');
            return;
        }
