...
server.registerTool(
    ...
    {
        description:
            "Export the geometry graph as NGE-compatible JSON. This JSON can be loaded in the Babylon.js Node Geometry Editor " + "or via NodeGeometry.Parse() at runtime.",
        inputSchema: {
...
server.registerTool(
    ...
    {
        description: "Import an existing NGE JSON into memory for editing. You can then modify blocks, connections, etc.",
        inputSchema: {
            geometryName: z.string().describe("Name to give the imported geometry"),
            json: z.string().describe("The NGE JSON string to import"),
        },
    },
    async ({ geometryName, json }) => {
        const result = manager.importJSON(geometryName, json);
        if (result !== "OK") {
            return { content: [{ type: "text", text: `Error: ${result}` }], isError: true };
        }
        const desc = manager.describeGeometry(geometryName);
