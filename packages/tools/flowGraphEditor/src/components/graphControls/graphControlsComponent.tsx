...
export const GraphControlsComponent: FunctionComponent<IGraphControlsProps> = (props) => {
    ...
    useEffect(() => {
        ...
        const debugObs = globalState.onDebugModeChanged.add((debugModeValue) => setDebugMode(debugModeValue));
        const liveObs = globalState.onLiveValidationChanged.add((liveValidationValue) => setLiveValidation(liveValidationValue));
        const validationObs = globalState.onValidationResultChanged.add((validationResultValue) => setValidationResult(validationResultValue));
        ...
        const timeScaleObs = globalState.onTimeScaleChanged.add((timeScaleValue) => setTimeScale(timeScaleValue));
    ...
    const onValidate = useCallback(() => {
        ...
        if (result && result.issues.length > 0) {
            ...
            const maxIssues = 20;
            for (let issueIndex = 0; issueIndex < Math.min(result.issues.length, maxIssues); issueIndex++) {
    ...
    const selectedContext = contextList.find((context) => context.index === selectedContextIndex);
            ...
            <div className={classes.timeScale}>
                ...
                {SpeedPresets.map((speedPreset) => (
                    <Button
                        ...
                        size="small"
                        appearance={timeScale === speedPreset ? "primary" : "subtle"}
                        title={`${speedPreset}× speed`}
                        onClick={() => {
                            globalState.timeScale = speedPreset;
                        }}
                    >
                        {speedPreset}×
                    </Button>
